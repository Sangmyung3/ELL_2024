...
## Partially Linear Model with 1 lags as linear part
import os
import time
import pickle
import sqlite3
...
for seed in seed_list:
    ...
    for cv_i, max_depth in enumerate(max_depth_list):
        ...
        beta = (1/(resi_v.T @ resi_v)) * (resi_v.T @ resi_y)
        resi_train = Y_train.values - V_train.values * beta
    ...
    V_train_one = np.concatenate((np.ones((n_train,1)),V_train.values.reshape(-1,1)),axis=1)
    ...
    for cv_j, lam in enumerate(lambda_list):
        ...
        Y_hat = np.concatenate((np.ones((n_val,1)),V_val.values.reshape(-1,1)),axis=1)@beta_hat + K_val@alpha_hat
    ...
    Y_hat = np.concatenate((np.ones((n_test,1)),V_test.values.reshape(-1,1)),axis=1)@beta_hat_dict[min_idx] + K_test@alpha_hat_dict[min_idx]
    ...
    for cv_i, n_node in enumerate(n_node_list):
        ...
        resi_v = V_train.values - model1.predict(W_train_nnan,verbose=0).reshape(-1,)
